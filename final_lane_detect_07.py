...
class ImageProcessor:  # 1단계: 클래스 이름 정의
    ...
    def crop_img_left(self, hsv_img):
        ...
        croped_img_left = hsv_img[hsv_img.shape[0] * 5 // 6 : hsv_img.shape[0], 0 : hsv_img.shape[1] // 2]  # 좌측에 있는 차선 영역을 추출합니다.
    ...
    def crop_img_right(self, hsv_img):
        # 이미지를 자르는 메서드로, 차량 운행 중 관심 영역만을 추출합니다.
        croped_img_right = hsv_img[hsv_img.shape[0] * 5 // 6 : hsv_img.shape[0], hsv_img.shape[1] // 2 : hsv_img.shape[1]]  # 우측에 있는 차선 영역을 추출합니다.
    ...
    def create_trackbar_init(self, cv_img):
        ...
        cv2.createTrackbar("Low_V_2", self.cropped_window, 118, 255, hsv_track_group2)
